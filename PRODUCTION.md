...
| **Supabase (ฐานข้อมูลจริง)** | ⏳ schema/migrations พร้อม | สร้างโปรเจกต์ + รัน schema + ใส่ env + ต่อ repo mapping ฟิลด์ใหม่ (ดูหัวข้อ Supabase) |
...
- ⚠️ **ยังเหลือก่อนสลับไป Supabase จริง:** `src/lib/supabase/repo.ts` + `profileToUser` (ใน `store.tsx`) ยัง **ไม่ได้ map ฟิลด์ใหม่** เหล่านี้ — ที่อยู่/พื้นที่ศูนย์คัดแยก, พื้นที่ตู้, owner/permissions ผู้ดูแล, และการผูกราคาของเก่า (`material_prices`) กับหน้าตีราคา ตอนนี้ฟีเจอร์เหล่านี้ครบเฉพาะ **โหมดเดโม (localStorage)**; ถ้าเปิด Supabase ต้องต่อ mapping ให้ครบก่อน (งานนี้ทดสอบได้ต่อเมื่อมีโปรเจกต์ Supabase จริง)
