...
- **โหมด Supabase ใช้ SMS OK ด้วย** (ไม่ใช้ OTP ในตัว Supabase ที่ต้องมี Twilio): สมัคร → ยืนยัน OTP SMS OK → สร้าง auth user ผ่าน `POST /api/auth/register` (service_role, `phone_confirm`) → sign in ด้วยเบอร์+รหัส · ลืมรหัส → `POST /api/auth/reset-password`
...
- **⚙️ ตั้งค่า Supabase Auth (จำเป็นสำหรับล็อกอินด้วยเบอร์):** Authentication → Sign In / Providers → **เปิด Phone provider** · **ไม่ต้อง**ตั้ง SMS provider (Twilio ฯลฯ) เพราะเรายืนยันเบอร์เอง (`phone_confirm`) + ส่ง OTP ผ่าน SMS OK
- ทดสอบจริง: เปิด `/register` → กรอกเบอร์ตัวเอง → รับ SMS → กรอกโค้ด → เข้าระบบ
